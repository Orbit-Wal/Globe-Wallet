...
export const environmentSchema = z.object({
  ...
  AUTH_JWT_SECRET: optionalNonEmptyString(z.string().min(32)),
  ...
  BASE_RPC_URL: optionalNonEmptyString(z.string().url()),
  ETHEREUM_RPC_URL: optionalNonEmptyString(z.string().url()),
  EVM_SOURCE_PRIVATE_KEY: optionalNonEmptyString(z.string().regex(/^0x[0-9a-fA-F]{64}$/, 'must be a 32-byte hex private key (starts with 0x, 64 hex chars)')),
  SOLANA_RPC_URL: optionalNonEmptyString(z.string().url()),
  SOLANA_SOURCE_PRIVATE_KEY: optionalNonEmptyString(z.string().min(1)),
  SUI_RPC_URL: optionalNonEmptyString(z.string().url()),
  SUI_SOURCE_PRIVATE_KEY: optionalNonEmptyString(z.string().min(1)),
})
